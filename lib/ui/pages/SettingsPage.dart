...
class SettingsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    initSetting() async {
      videoUrl.value =
          TextEditingValue(text: await Store.getString(_videoUrlKey));
      baseUrl.value =
          TextEditingValue(text: await Store.getString(_baseUrlKey));
    }
                      ...
                      onPressed: () async {
                        if (baseUrl.value.text.isEmpty) {
                          ...
                          await Store.setString(
                              _baseUrlKey, baseUrl.value.text);
                        }
                        if (videoUrl.value.text.isEmpty) {
                          await Store.remove(_videoUrlKey);
                        } else {
                          await Store.setString(
                              _videoUrlKey, videoUrl.value.text);
                        }
